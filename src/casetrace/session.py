    ...
    async def take_control(self, intervention_id: str) -> None:
        async with self._action_lock:
            async with self._state_lock:
                ...
                if self.state is not SessionState.PAUSING or intervention.state != "waiting_human":
        ...
        async with self._action_lock:
            ...
            async with self._state_lock:
                ...
                if not allowed:
                    ...
                    self.ownership_generation += 1
                    self._intervention = intervention.model_copy(
                        update={"reason": reason, "ownership_generation": self.ownership_generation}
                    )
                    await self._notify_transition()
                    raise RunStopped(
    ...
    async def abort(self) -> None:
        async with self._state_lock:
            ...
            if self._intervention is not None:
                self._intervention = self._intervention.model_copy(update={"state": "aborted"})
            await self._notify_transition()
